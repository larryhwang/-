...
#define SingleBtnWidth SCREEN_WIDTH/2
...
@interface SCNavTabBar () <SCPopViewDelegate>
{
    ...
    UIView          *_lineBack;
    UIView          *_VerticalLine;      //vertical line
...
{
#warning 初始条状背景显示控制
    _lineBack = [[UIView alloc] initWithFrame:CGRectMake(2.0f, NAV_TAB_BAR_HEIGHT - 3.0f, SCREEN_WIDTH, 2.0f)];
    _lineBack.backgroundColor = [UIColor grayColor];
    
    _line = [[UIView alloc] initWithFrame:CGRectMake(33.75f, NAV_TAB_BAR_HEIGHT - 3.0f, 87.5f, 2.0f)];
    _line.backgroundColor = UIColorWithRGBA(20.0f, 80.0f, 200.0f, 0.7f);
    
    
    _VerticalLine = [[UIView alloc]initWithFrame:CGRectMake(SingleBtnWidth, 8, 1, NAV_TAB_BAR_HEIGHT - 16)];
    _VerticalLine.backgroundColor = [UIColor grayColor];
    
    
    [_navgationTabBar addSubview:_lineBack];
    [_navgationTabBar addSubview:_line];
    [_navgationTabBar addSubview:_VerticalLine];
...
- (void)setCurrentItemIndex:(NSInteger)currentItemIndex
{
    ...
    [UIView animateWithDuration:0.2f animations:^{
        // _line = [[UIView alloc] initWithFrame:CGRectMake(33.75f, NAV_TAB_BAR_HEIGHT - 3.0f, 87.5f, 3.0f)];
...
#warning 动态划线
    ...
    _line.frame = CGRectMake(button.frame.origin.x + SingleBtnWidth /4, _line.frame.origin.y, [_itemsWidth[currentItemIndex] floatValue]/2 , _line.frame.size.height);
...
- (void)updateData
{
    ...
    /*
     ...
     NSNumber *width = [NSNumber numberWithFloat:175.f];
     //  NSArray *itemWidths = [self getButtonsWidthWithTitles:itemNames];
     NSArray *itemWidths = [NSArray arrayWithObjects:width,width,nil];
     */
    
#warning 宽度数组
    
     NSNumber *width = [NSNumber numberWithFloat:SCREEN_WIDTH/2];
     NSArray *itemsWidth = [NSArray arrayWithObjects:width,width,nil];
  //  _itemsWidth = [self getButtonsWidthWithTitles:_itemTitles];
    _itemsWidth = itemsWidth;
    if (itemsWidth.count)
    {
        CGFloat contentWidth = [self contentWidthAndAddNavTabBarItemsWithButtonsWidth:itemsWidth];
